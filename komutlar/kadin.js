...
exports.run = (client, message, args) => {
  ...
  const genelrol = message.guild.roles.find(r => r.id === "772116923634548736"); 
  const kız = message.guild.roles.find(r => r.id === "772116923634548736"); 
  const misafir = message.guild.roles.find(r => r.id === "772119728809574421"); 
  const log = message.guild.channels.find(c => c.id === "772121717350334485"); 
  const tag = "TAG";
  if(!message.member.roles.array().filter(r => r.id === "773998334947098644")[0]) { 
